...
- [x] (2026-03-27 16:26Z) Simplified the UX so the organization input remains visible and the suggested organization is written directly into the field as the selected default value, with no explicit suggestion panel or `Change organization` action.
...
- The UX can be simplified even further: no explicit suggestion chrome is necessary if the organization field stays visible and the suggested org is written directly into the field while the selected organization ID is set behind the scenes.
...
- Decision: Remove explicit suggestion UI such as a lookup panel or `Change organization` button, and instead keep the organization field visible with the suggested organization written directly into the field as the selected default value.
...
- If an org is suggested, the organization field is populated with that org as the default selection without additional suggestion UI.
...
1. Enter a valid email with a suggestable domain and verify the suggested organization is auto-selected and written directly into the organization field.
