...
- [x] (2026-07-13T19:05Z) Routed service validation errors to their actual fields and avoided patching empty contributors immediately after first create.
- [x] (2026-07-13T19:15Z) Added error-state styling to the shared search input and applied it to the contributor search box.
- [x] (2026-07-13T19:20Z) Updated builder tab locking so Outcomes/Materials unlock once the learning object exists instead of waiting for every save/submission field to be valid.
...
Follow-up refinements: required Basic Info fields now use small red asterisks instead of Required badges, name/description info icons were removed, academic level labels are display-title-cased while stored values remain lowercase, contributor search shows a red error state when contributor validation is active, Outcomes/Materials unlock once a named learning object has been created, the builder length dropdown cannot clear the current selection, length changes remain visually selected while store updates and other inline validation settle, first-create preserves and then patches local length/level/description/material/contributor edits made before naming, service validation errors are routed to their actual fields, Children is visible for non-nano lengths but only searches after a learning object exists, nanomodules hide the Children scaffold, the description rich-text editor has local styling for its toolbar/buttons/editor area, full-width layout, and a shorter body, the builder pages remain fixed below the fixed navbar using the original calculated sizing behavior, missing-name validation waits until the user touches the name field or attempts to continue, and duplicate/current-page validation is shown inline without a duplicate toaster.
