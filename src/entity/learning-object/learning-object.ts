...
export class LearningObject {
    ...
    private isValidLength(length: LearningObject.Length): boolean {
        const validLengths: LearningObject.Length[] = Object.keys(
            ...
            (key: string) =>
                LearningObject.Length[key] as LearningObject.Length,
        );
        ...
        if (object) {
            const addingObject =
                object instanceof LearningObject
                    ? object
                    : new LearningObject(object);
            this.updateDate();
            return this._children.push(addingObject) - 1;
        } else {
            throw new EntityError(
                LEARNING_OBJECT_ERRORS.INVALID_CHILD,
                "children",
            );
        }
    }
    ...
    private isValidStatus(status: LearningObject.Status): boolean {
        const validStatuses: LearningObject.Status[] = Object.keys(
            ...
            (key: string) =>
                LearningObject.Status[key] as LearningObject.Status,
        );
    ...
    private copyObject(object: Partial<LearningObject>): void {
        ...
        if (object.children) {
            (object.children as LearningObject[]).map((child) =>
                this.addChild(child),
            );
        }
        ...
        this.metrics =
            (object.metrics as LearningObject.Metrics) || this.metrics;
    }
