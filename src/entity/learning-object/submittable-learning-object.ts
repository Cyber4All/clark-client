...
export namespace SubmittableLearningObject {
    ...
    const NON_TERMINATING_HTML_REGEX =
        /<(?!(br\b))([a-z]+)(\s[^>]*)?(?<!\/)>(?![\s\S]*<\/\2>)/gi;
    ...
    export function validateDescription(description: string) {
        ...
        if (description.match(NON_TERMINATING_HTML_REGEX)) {
            throw new EntityError(
                SUBMITTABLE_LEARNING_OBJECT_ERRORS.INVALID_DESCRIPTION_BAD_HTML,
                "description",
            );
        }
    }
    ...
    export function validateNotes(notes: string) {
        ...
        if (notes.match(NON_TERMINATING_HTML_REGEX)) {
            throw new EntityError(
                SUBMITTABLE_LEARNING_OBJECT_ERRORS.INVALID_NOTES_BAD_HTML,
                "notes",
            );
        }
    }
    ...
    export function validateContributors(contributors: User[]) {
        if (contributors.length <= 0) {
            throw {
                ...
                message: "At least one contributor must be added to the object",
            };
        }
    }
