...
export class LearningObjectService {
     ...
     */
    getLearningObject(cuid: string, version?: number): Promise<LearningObject> {
        const route = LEARNING_OBJECT_ROUTES.GET_LEARNING_OBJECT(cuid, version);
    ...
    fetchLearningObject(cuid: string, version: number): Promise<any> {
        return this.http
            .get(LEARNING_OBJECT_ROUTES.GET_LEARNING_OBJECT(cuid, version), {
    ...
    fetchLearningObjectWithResources(
        ...
        this.getLearningObjectObservable(params)
            .pipe(take(1))
            .subscribe((object) => {
                // TODO: I don't think this if is ever true because
                if (object instanceof LearningObject) {
                    this.fetchLearningObjectResources(object, resources)
                        .pipe(
    ...
    fetchLearningObjectResources(
        object: LearningObject,
        resources: string[],
    ): Observable<{ name: string; data: any }> {
        const resourceUris: { [key: string]: string } = {};
        if (object.resourceUris !== undefined) {
            Object.keys(object.resourceUris)
                .filter((x) => resources.includes(x))
                .map((key) => {
        ...
        return merge(
            ...Object.entries(resourceUris).map(([name, uri]) => {
    ...
    fetchUri(uri: string, callback?: Function) {
        return this.http
            .get(uri, { headers: this.headers, withCredentials: true })
            .pipe(
                take(1),
    ...
    async getLearningObjectChildren(
        learningObjectId: string,
    ): Promise<LearningObject[]> {
        return this.http
            .get(
                LEARNING_OBJECT_ROUTES.GET_LEARNING_OBJECT_CHILDREN(
                    learningObjectId,
                ),
            ...
            .then((learningObjects: any[]) => {
                return learningObjects.map(
                    (learningObject) => new LearningObject(learningObject),
                );
            });
    }

    async getLearningObjectParents(
        learningObjectId: string,
    ): Promise<LearningObject[]> {
        return this.http
            .get(
    ...
    async updateLearningObjectStatus(
        learningObjectId: string,
        ...
        return this.http
            .post(
                LEARNING_OBJECT_ROUTES.UPDATE_LEARNING_OBJECT_STATUS(
                    learningObjectId,
                ),
                { status, reason },
                { withCredentials: true },
    ...
    async addChildren(
        ...
        return await this.http
            .post(
                LEARNING_OBJECT_ROUTES.UPDATE_LEARNING_OBJECT_CHILDREN(
                    object._id,
                ),
                {
                    childrenIds,
    ...
    setChildren(
        ...
        const removeRoute =
            LEARNING_OBJECT_ROUTES.REMOVE_LEARNING_OBJECT_CHILD(
                learningObjectId,
            );
        const addRoute =
            LEARNING_OBJECT_ROUTES.UPDATE_LEARNING_OBJECT_CHILDREN(
    ...
    delete(learningObjectId: string): Promise<{}> {
        const route =
            LEARNING_OBJECT_ROUTES.DELETE_LEARNING_OBJECT(learningObjectId);
        return this.http
            .delete(route, {
                headers: this.headers,
                withCredentials: true,
    ...
    deleteMultiple(learningObjectIds: string[]): Promise<any> {
        const deletePromises = learningObjectIds.map((objectId) =>
            this.http
                .delete(
                    LEARNING_OBJECT_ROUTES.DELETE_LEARNING_OBJECT(objectId),
                    {
                        headers: this.headers,
                        withCredentials: true,
                        responseType: "text",
