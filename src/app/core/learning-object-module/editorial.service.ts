...
export class EditorialService {
    ...
    navigateToEditor(
        ...
        window.open(
            `onion/learning-object-builder/${learningObject.cuid}/${params?.version ?? learningObject.version}`,
            "_blank",
        );
    }
    ...
    canMakeEdits(
        ...
        return (
            ...
            (revisedLearningObject &&
                revisedLearningObject.status === "waiting") ||
    ...
    canCreateRelevancyStory() {
        return (
            this.auth.user.accessGroups.includes("admin") ||
            this.auth.user.accessGroups.includes("editor")
        );
    }
    ...
    canMapAndTag(learningObject: LearningObject) {
        const userIsAuthor =
            learningObject.author.username === this.auth.username;
        const untaggable =
            learningObject.status === LearningObject.Status.UNRELEASED;

        if (
            this.auth.user &&
            this.auth.user.accessGroups &&
            !userIsAuthor &&
            !untaggable
        ) {
            ...
            if (
                this.auth.user.accessGroups.some((priv) =>
