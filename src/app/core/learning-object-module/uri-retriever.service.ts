...
export class UriRetrieverService {
    ...
    getLearningObject(
        ...
        try {
            const request =
                this.learningObjectService.getLearningObjectResources(
                    params,
                    resources || [],
                );
     ...
     */
    getLearningObjectChildren(params: {
        uri: string;
        unreleased?: boolean;
    }): Observable<LearningObject[]> {
        if (params.unreleased) {
            return this.http
                .get<LearningObject[]>(params.uri, { withCredentials: true })
                .pipe(take(1));
        } else {
            return this.http
                .get<LearningObject[]>(params.uri, { withCredentials: true })
    ...
    private setRoute(params: {
        ...
        if (params.cuidInfo?.cuid) {
            route = LEARNING_OBJECT_ROUTES.GET_LEARNING_OBJECT(
    ...
    private userError(params: {
        ...
        if (params.author && !params.name) {
            ...
            return new Error(
                "Cannot find Learning Object. No identifiers found.",
            );
        }
    }
}
