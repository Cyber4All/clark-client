    ...
    async getFeaturedObjects() {
        ...
        const promisedObjectsArray = objects.map(
            async (learningObject: LearningObject) => {
                ...
                const object =
                    await this.learningObjectService.fetchLearningObject(
                        learningObject.cuid,
                ...
                if (object.resourceUris?.outcomes) {
                    ...
                    console.log(
                        "FIX ME: No outcomes found for learning object",
                    );
                }
                return object;
            },
        ...
        this.featuredStore.featured = await Promise.all(
            promisedObjectsArray,
        ).then(async (learningObject: LearningObject[]) => {
            return learningObject;
        });
        // Save the array into the _featuredObjects subject to be observed
        this._featuredObjects$.next(
            Object.assign({}, this.featuredStore).featured,
        );
        this.setSubmitError();
    }
    ...
    async saveFeaturedObjects() {
        ...
        this.featuredStore.featured.forEach((obj) => {
            featuredDto.push({
                cuid: obj.cuid,
                version: obj.version,
                featuredCollection: obj.collection,
            });
        });
        if (this.featuredStore.featured.length !== 5) {
            this._submitError$.next(true);
