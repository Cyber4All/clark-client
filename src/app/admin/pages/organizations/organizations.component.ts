...
export class OrganizationsComponent
    ...
    readonly levelFilterOptions: DropdownFilterOption[] =
    ...
    ngOnInit(): void {
        ...
        this.dataSource.filterPredicate = (
            ...
            const matchesSector =
                ...
                sectorFilters.includes(data.sector);
            ...
            .pipe(
                switchMap((firstPage: SearchOrganizationsResponse) => {
                    ...
                    const pageRequests: Array<
                        ReturnType<
                            OrganizationService["searchOrganizationsResponse"]
                        >
                    > = [];
                    for (let page = 2; page <= totalPages; page++) {
                        pageRequests.push(
                            this.organizationService.searchOrganizationsResponse(
                                { page, limit, status: statuses },
                            ),
                        );
                    }
                    ...
                    return forkJoin(pageRequests).pipe(
                        map((responses: SearchOrganizationsResponse[]) => {
                            ...
                            firstOrganizations.forEach((org) =>
    ...
    toggleOrganizationVerification(org: Organization): void {
        ...
        this.organizationService
            ...
            .subscribe({
                ...
                error: (error) => {
                    console.error(
                        "Error updating organization verification:",
                        error,
                    );
                    this.toaster.error(
                        "Error",
    ...
    private refreshExistingNames(): void {
        this.existingNames = this.dataSource.data
            .filter(
                (org): org is Organization => !!org && typeof org === "object",
            )
            .map((org) => {
        ...
        this.organizationService
            ...
            .subscribe({
                next: (response) => {
                    ...
                    this.learningObjectCountMap.set(
    ...
    private async loadTotalOtherUsers(): Promise<void> {
        ...
        try {
            ...
            const otherOrganization = organizations.find(
                (org) =>
                    (org.normalizedName || "").trim().toLowerCase() === "other",
            );
    ...
    private async loadVisibleOrganizationCounts(): Promise<void> {
        ...
        const visibleOrganizationIds = visibleOrganizations.map(
            (org) => org._id,
        );
        ...
        const missingLearningObjectCountIds = visibleOrganizationIds.filter(
            (organizationId) =>
                !this.learningObjectCountMap.has(organizationId),
        );
        ...
        if (missingLearningObjectCountIds.length > 0) {
            const learningObjectCountPairs = await this.fetchCountPairs(
                ...
                (organizationId) =>
                    this.fetchLearningObjectCount(organizationId),
            );
            ...
            const chunkCounts = await Promise.all(
                chunk.map(
                    async (organizationId) =>
                        [organizationId, await fetcher(organizationId)] as [
                            string,
                            number,
                        ],
                ),
    ...
    onMigrateUsers(targetOrgId: string): void {
        ...
        this.organizationService
            .migrateOrganizationUsers(sourceOrg._id, {
