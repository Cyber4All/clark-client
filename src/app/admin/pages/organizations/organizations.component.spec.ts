...
describe("OrganizationsComponent", () => {
    ...
    let organizationService: ReturnType<
        typeof jasmine.createSpyObj<OrganizationService>
    >;
    ...
    beforeEach(() => {
        toaster = jasmine.createSpyObj<ToastrOvenService>("ToastrOvenService", [
            "success",
            "error",
        ]);
    ...
    it("returns verified target organizations excluding the selected source org", () => {
        const sourceOrganization = buildOrganization({
            _id: "source-org",
            name: "Source University",
        });
        const verifiedTarget = buildOrganization({
            _id: "verified-target",
        ...
        expect(component.getAvailableTargetOrganizations()).toEqual([
            verifiedTarget,
        ]);
    });
    ...
    it("migrates users, clears cached counts, and refreshes organizations on success", () => {
        ...
        const targetOrganization = buildOrganization({
            _id: "target-org",
        ...
        expect(
            organizationService.migrateOrganizationUsers,
        ).toHaveBeenCalledWith(sourceOrganization._id, {
            ...
            throwError(
                () =>
                    new HttpErrorResponse({
                        status: 400,
                        error: {
                            message: "Target organization must be verified.",
                        },
                    }),
            ),
        );
