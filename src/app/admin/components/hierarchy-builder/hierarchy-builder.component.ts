...
export class HierarchyBuilderComponent implements OnInit {
    ...
    treeControl = new NestedTreeControl<LearningObjectNode>(
        (node) => node.children,
    );
    ...
    async createLearningObjects(node: any) {
        ...
        if (node.children.length === 0) {
            return await this.hierarchyService.addHierarchyObject(
                this.parent.author.username,
                node,
            );
        }
        ...
        for await (const child of node.children) {
            const obj = await this.createLearningObjects(child);
            childrenIds.push(obj._id);
        }

        if (node.name !== this.parent.name) {
            const obj = await this.hierarchyService.addHierarchyObject(
                this.parent.author.username,
                node,
            );
            finalObj = obj;
        ...
        const newNode = await this.setParents(finalObj || node, childrenIds);
        if (node.name === this.parent.name) {
            this.toaster.success("Success", "Hierarchy Created!");
            this.close.emit();
        }
        return newNode;
    }
    ...
    async setParents(node, childs) {
        await this.learningObjectService.addChildren(
            this.parent.author.username,
            node,
            childs,
        );
