...
export class ChangeAuthorComponent implements OnInit {
    ...
    async changeAuthor() {
        const author: User = await this.userService.getUser(
            this.highlightedLearningObject.author._username,
        );
        this.changeAuthorshipService
            .changeAuthorship(
                author.userId,
                this.highlightedLearningObject.id,
            ...
            .then(
                () => {
                    this.toaster.success(
                        "Success!",
                        "Learning Object Author changed.",
                    );
                    this.close.emit();
                },
                (error) => {
                    this.toaster.error(
                        "Error!",
                        "Unable to change authorship at this time.",
                    );
                },
            );
    }
