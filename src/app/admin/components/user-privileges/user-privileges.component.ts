...
export class UserPrivilegesComponent implements OnInit {
    ...
    private getUserRoles() {
        this.accessGroupService
            .getUserAccessGroups(this.user.userId)
            .then((roles) => {
            ...
            .catch((error) => {
                this.toaster.error(
                    "Error!",
                    "There was an error fetching this user's privileges. Please try again later.",
                );
            });
    }
        ...
        if (collectionIndex === -1) {
            ...
            this.accessGroupService
                ...
                .catch((error) => {
                    this.toaster.error(
                        "Error!",
                        "There was an error adding a privilege. Please try again later.",
                    );
                });
        }
    }
    ...
    addMapper() {
        this.accessGroupService
            .addAccessGroupToUser(this.user.username, AccessGroups.MAPPER)
            .then(() => {
                try {
                    this.toaster.success(
                        "Success!",
                        "User promoted to Mapper.",
                    );
                } catch (e) {}
            ...
            .catch((error) => {
                this.toaster.error(
                    "Error!",
                    "There was an error promoting the user. Please try again later.",
                );
            });
    }
    ...
    async remove(index: number) {
        ...
        this.accessGroupService
            .removeAccessGroupFromUser(this.user.username, role, collection)
            .then(() => {
            ...
            .catch((error) => {
                this.toaster.error(
                    "Error!",
                    "There was an error removing a privilege. Please try again later.",
                );
            });
    }
    ...
    get collectionsWithSelectedRole() {
        if (this.selectedRole) {
            return this.privileges
                .filter((x) => x[0] === this.selectedRole)
                .map((x) => x[1]);
        }
    }
}
