...
export class LearningObjectListItemComponent implements OnChanges {
    ...
    goToUrl(url) {
        if (url === "builder") {
            ...
            this.router.navigate([
                `/users/${this.learningObject.author.username}`,
            ]);
        } else if (url === "details") {
            this.router.navigate([
                `/details/${this.learningObject.author.username}/${this.learningObject.cuid}/${this.learningObject.version}`,
            ]);
        } else if (url === "revision builder") {
            this.editorialService.navigateToEditor(this.learningObject, {
                version: this.learningObject.version + 1,
            });
        }
    }
    ...
    releaseHierarchy() {
        ...
        this.learningObjectService
            .releaseHierarchy(this.learningObject.id)
            .then(() => {
    ...
    deleteRevision() {
        this.editorialService
            ...
            .then(() => {
                this.toaster.success(
                    "Success",
                    "Learning object unreleased revision deleted successfully",
                );
            })
            .catch(() => {
                this.toaster.error(
                    "Error",
                    "There was an issue deleting the revision of this learning object, please try again later",
                );
            });
