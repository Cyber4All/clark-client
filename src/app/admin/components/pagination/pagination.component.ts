...
export class PaginationComponent implements OnInit {
    ...
    getPageNumberList() {
        const { leftSideCount, rightSideCount } =
            this.calculateSurroundingPageNumbers();
    ...
    getCappedRemainingPageCount() {
        ...
        // Never display more than 2 page numbers on the right
        // of the selected page number
        const cappedRemainingPageCount =
            remainingPageCount > 2 ? 2 : remainingPageCount;
    ...
    getCappedPreviousPageCount() {
        ...
        const cappedPreviousPageCount =
            this.currentPageNumber - 1 > 2 ? 2 : this.currentPageNumber - 1;
    ...
    calculateSurroundingPageNumbers() {
        ...
        if (rightSideCount === 0 && this.currentPageNumber > 4) {
            leftSideCount = 4;
        } else if (rightSideCount === 1 && this.currentPageNumber >= 4) {
            leftSideCount = 3;
        }
        ...
        if (leftSideCount === 0 && this.lastPageNumber >= 5) {
            rightSideCount = 4;
        } else if (leftSideCount === 1 && this.lastPageNumber >= 5) {
            rightSideCount = 3;
        }
