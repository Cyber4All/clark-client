...
export class ClarkComponent implements OnInit {
    ...
    constructor(
        ...
        this.router.events
            .pipe(
                filter(
                    (event) => event instanceof NavigationEnd && event.id > 1,
                ),
            )
            .subscribe(() => {
        ...
        this.route.queryParams.subscribe(() => {
            if (this.route.snapshot.queryParams.err) {
                this.toaster.error(
                    "SSO Error",
                    decodeURIComponent(this.route.snapshot.queryParams.err),
                );
            }
        });
    }
    ...
    displayCookieBanner() {
        return (
            this.cookieAgreement.getShowCookieBannerVal() &&
            !this.cookieAgreement.getCookieAgreementVal()
        );
    }
  ...
  browser tabs and notify AT's of current location */
    setPageTitle() {
        this.router.events
            ...
            .subscribe(() => {
                let data;
                const activeRoutes: ActivatedRoute[] =
    ...
    shouldShowChatbot(): boolean {
        return !this.router.url.includes("/onion");
    }
}
