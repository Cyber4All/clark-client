...
export class AlignmentService {
    ...
    guidelines$: Observable<SearchItemDocument[]> =
        this.guidelines.asObservable();
    ...
    async saveOutcomeMappings(
        oldOutcomes: LearningOutcome[],
        learningObjectId: string,
    ) {
        // See what outcomes have been updated
        const updatedOutcomes: LearningOutcome[] = this.diffOutcomeArrays(
            oldOutcomes,
            this.outcomes.value,
            "id",
        );
        ...
        for (const outcome of updatedOutcomes) {
            ...
            const outcomeMappingIds = outcome.mappings.map(
                (mapping) => mapping.id,
            );
