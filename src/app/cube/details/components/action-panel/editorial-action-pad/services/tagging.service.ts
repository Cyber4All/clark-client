...
export class TaggingService {
     ...
     */
    setSourceArray(
        arrayName: "topics" | "tags",
        sourceArray: Topic[] | Tag[],
    ): void {
        if (arrayName === "topics") {
    ...
    addItemToArray(arrayName: "topics" | "tags", item: Topic | Tag): void {
        ...
        if (arrayName === "topics") {
            if (
                (sourceArray as Topic[]).some(
                    (topic) => topic._id === (item as Topic)._id,
                ) &&
                !(currentSelected as Topic[]).some(
                    (selected) => selected._id === (item as Topic)._id,
                )
            ) {
                const updatedSelected = [
                    ...(currentSelected as Topic[]),
                    item as Topic,
                ];
    ...
    removeItemFromArray(arrayName: "topics" | "tags", item: Topic | Tag): void {
        ...
        if (arrayName === "topics") {
            ...
            const updatedSelected = (currentSelected as Tag[]).filter(
