...
export class ActionPanelComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit(): Promise<void> {
        this.userIsAuthor =
            this.learningObject.author.username === this.auth.username;
        this.auth.group.pipe(takeUntil(this.destroyed$)).subscribe(() => {
            this.userCanRevise =
                this.auth.hasEditorAccess() || this.userIsAuthor;
            this.hasReviewerAccess = this.auth.hasReviewerAccess();
        });
        this.hasDownloadAccess =
            (this.hasReviewerAccess || this.isReleased) &&
            this.auth.user !== null;
    ...
    copyAttribution() {
        ...
        this.toaster.success(
            "Success!",
            "Attribution information has been copied to your clipboard!",
        );
    }
    ...
    shareButton(event, type) {
        switch (type) {
            ...
            case "twitter":
                const text =
                    "Check out this learning object on CLARK! ClarkCan";
                window.open(
            ...
            case "linkedin":
                const payload = {
                    comment:
                        "Check out this learning object on CLARK! " +
                        this.url +
                        " #ClarkCan",
                    visibility: {
                        code: "anyone",
    ...
    getContributorAttribution() {
        ...
        if (
            ...
            this.learningObject.contributors.length >= 3
        ) {
            // 3 or more contributors: 'a, b, and c'
            for (let i = 0; i < this.learningObject.contributors.length; i++) {
                const name = this.capitalizeName(
                    this.learningObject.contributors[i].name,
                );

                attribution +=
                    i === this.learningObject.contributors.length - 1
                        ? "and " + name
                        : name + ", ";
            }
        } else if (
            ...
            attribution = this.capitalizeName(
                this.learningObject.contributors[0].name,
            );
            if (this.learningObject.contributors.length === 2) {
                attribution +=
                    " and " +
                    this.capitalizeName(
                        this.learningObject.contributors[1].name,
                    );
            }
        } else {
