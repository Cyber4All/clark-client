...
export class LearningObjectRatingsComponent implements OnInit {
    ...
    @Output() respondRating: EventEmitter<{ comment: string }> =
        new EventEmitter();
    @Output() reportRating: EventEmitter<{
        concern: string;
        index: number;
        comment?: string;
    }> = new EventEmitter();
    ...
    calculateAverageRating(): number {
        if (this.ratings.length > 0) {
            return (
                this.ratings.map((x) => x.value).reduce((x, y) => x + y) /
                this.ratings.length
            );
        }
    }
    ...
    isResponseAuthor(index: number): boolean {
        if (
            this.ratings[index].response[0].user.username === this.auth.username
        ) {
            return true;
        }
        return false;
