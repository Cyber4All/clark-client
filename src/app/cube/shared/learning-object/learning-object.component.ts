...
export class LearningObjectListingComponent implements OnDestroy {
    ...
    private async refresh(): Promise<void> {
        ...
        this.isTrending =
            (recentDownloads ?? 0) >=
            LearningObjectListingComponent.TRENDING_THRESHOLD;
    ...
    truncateText(text: string, max: number = 150, margin: number = 10): string {
        ...
        const spaceAfter =
            cleaned.substring(max).indexOf(" ") + truncated.length;
    ...
    private checkDCWFAlignment(lo: LearningObject): boolean {
        ...
        return lo.guidelines.some(
            (guideline) =>
                guideline.source
                    ?.toLowerCase()
                    .includes("dod cyber workforce") ||
    ...
    private async getRecentDownloads(
        cuid: string,
        days: number,
    ): Promise<number> {
        const end = new Date();
    ...
    private async resolveTopicNames(lo: LearningObject): Promise<void> {
        ...
        const topics = await Promise.all(
            lo.topics.map(async (id: string) => {
                const topic = await (
                    await this.topicsStore.topics$(id)
                ).toPromise();
                return topic?.name;
            }),
        );
