...
export class GuidelineFilterComponent implements OnInit, OnDestroy {
    ...
    selectFrameworkOption(value: string) {
        const index = this.frameworkFilter.filters.findIndex(
            (filter) => filter.value === value,
        );
        this.frameworkFilter.filters.forEach(
    ...
    private getSelectedFramework(): string {
        const framework = this.frameworkFilter.filters.find(
    ...
    deselectGuidelineOption(guidelineId: string) {
        const index = this.selectedGuidelines.findIndex(
            (id) => id === guidelineId,
        );
        if (index >= 0) {
