...
export class BuildProgramComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.buildProgramComponentService.currentFrameworkObservable.subscribe(
            (framework) => {
    ...
    handleFrameworkClicked(event: string) {
        ...
        this.guidelineService
            ...
            .then((result) => {
                this.currentFrameworkGuidelines = result.results.sort((a, b) =>
                    this.sortGuidelines(a, b),
                );
                this.currentFrameworkGuidelinesFiltered = [
                    ...this.currentFrameworkGuidelines,
                ];
                this.fillPages();
            });
    ...
    filterGuidelineSearch() {
        this.currentFrameworkGuidelinesFiltered =
            this.currentFrameworkGuidelines.filter((guideline) => {
                return (
                    guideline.guidelineName.includes(this.guidelineSearch) ||
                    guideline.guidelineDescription.includes(
                        this.guidelineSearch,
                    )
                );
            });
    ...
    fillPages() {
        ...
        const numGuidelinesPerPage =
            window.outerWidth <= maxMobileWidth ? 3 : 6;
        // A 2D array of length equal to total number of pages needed
        const pages = new Array(
            Math.ceil(
                this.currentFrameworkGuidelinesFiltered.length /
                    numGuidelinesPerPage,
            ),
        );

        // Fills each index of the array with an array of guidelines
        for (let i = 0; i < pages.length; i++) {
            pages[i] = this.currentFrameworkGuidelinesFiltered.slice(
                i * numGuidelinesPerPage,
                i * numGuidelinesPerPage + numGuidelinesPerPage,
            );
        }
     ...
     */
    navigate(queryParams: { guidelines: string; standardOutcomes?: string[] }) {
        this.currentPage = 0;
        ...
        const params = {
            ...
            standardOutcomes: queryParams.standardOutcomes
                ? queryParams.standardOutcomes
                : [],
        };
