...
export class TeachNowComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit(): void {
        if (this.scroll) {
            this.scroll.nativeElement.addEventListener(
                "wheel",
                this.horizontalScroll.bind(this),
            );
        }
    }
    ...
    horizontalScroll(event) {
        ...
        if (
            this.scrollDistance >
            this.scroll.nativeElement.scrollWidth - window.outerWidth / 2 - 203
        ) {
            this.scrollDistance =
    ...
    getObjects() {
        this.loading = true;
        this.objects = this.loadingObjects;
        this.searchlearningObjectService
            .getLearningObjects({
                limit: 4,
                currPage: 1,
                status: ["released"],
    ...
    navigate() {
        this.router.navigate(["browse"], {
            queryParams: { topics: this.selectedTopic },
        });
    }
}
