...
@Component({
    ...
    animations: [
        trigger("isHighlighed", [
            ...
            transition("isHighlighted => notHighlighted", [animate("4s")]),
            transition("notHightlighted => isHighlighted", [animate("4s")]),
        ]),
    ],
    standalone: true,
...
export class StickyMenuComponent implements OnInit {
    ...
    setFields(selection: string) {
        this.isHighlighted.learningObjects =
            LEARNING_OBJECT_INFO_STATES.LEARNING_OBJECT === selection;
        this.isHighlighted.learningOutcomes =
            LEARNING_OBJECT_INFO_STATES.LEARNING_OUTCOMES === selection;
        this.isHighlighted.hierarchies =
            LEARNING_OBJECT_INFO_STATES.HIERARCHIES === selection;
    }
    ...
    isOtherChoiceSelected() {
        return Object.values(this.isHighlighted).every(
