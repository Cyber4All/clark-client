...
export class LearningObjectInfoComponent implements OnInit {
    ...
    isScrolledIntoView() {
        if (this.learningObjectDiv) {
            const rect =
                this.learningObjectDiv.nativeElement.getBoundingClientRect();
            if (this.isAtTopOfScreen(rect)) {
                this.currentComponent =
                    LEARNING_OBJECT_INFO_STATES.LEARNING_OBJECT;
                return;
            }
        }
        if (this.learningOutcomeDiv) {
            const rect =
                this.learningOutcomeDiv.nativeElement.getBoundingClientRect();
            if (this.isAtTopOfScreen(rect)) {
                this.currentComponent =
                    LEARNING_OBJECT_INFO_STATES.LEARNING_OUTCOMES;
                return;
            }
        }
        if (this.hierarchiesDiv) {
            const rect =
                this.hierarchiesDiv.nativeElement.getBoundingClientRect();
            if (this.isAtTopOfScreen(rect)) {
