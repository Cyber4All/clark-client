...
import {
    ...
    RatingService,
} from "app/core/rating-module/rating.service";
...
export class LibraryComponent implements OnInit, OnDestroy {
    ...
    async loadLibrary() {
        try {
            this.loading = true;
            const getLibraryResponse = await this.libraryService.getLibrary({
    ...
    async getNotifications(apiPage: number) {
        ...
        if (
            this.lastNotificationsPageNumber === this.localNotifications.length
        ) {
            this.currentNotificationsPageNumber =
                this.lastNotificationsPageNumber;
        } else {
    ...
    goBackNotifications() {
        if (this.firstIndex - this.notificationCardCount >= 0) {
            ...
            this.firstIndex =
                this.firstIndex -
                (this.localNotifications.length - this.notificationCardCount);
            this.lastIndex = this.firstIndex + this.notificationCardCount;
        }
    }
    ...
    async goForwardNotifications() {
        if (
            this.lastIndex + this.notificationCardCount <=
            this.localNotifications.length
        ) {
            this.firstIndex = this.firstIndex + this.notificationCardCount;
    ...
    async removeItem() {
        try {
            await this.libraryService.removeFromLibrary(
                this.libraryItemIdToDelete,
            );
            this.libraryItems = (
                await this.libraryService.getLibrary({ page: 1, limit: 10 })
            ).libraryItems;
    ...
    downloadObject(event: MouseEvent, object: LearningObject, index: number) {
        ...
        this.libraryService.downloadBundle(
            BUNDLING_ROUTES.DOWNLOAD_BUNDLE(object.id),
        );
    ...
    async changeLibraryItemPage(pageNumber: number) {
        this.topOfList.nativeElement.scrollIntoView({
            behavior: "smooth",
            block: "start",
        });
