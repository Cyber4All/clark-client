...
export class ProfileLearningObjectsComponent implements OnChanges {
    ...
    async ngOnChanges() {
        this._learningObjects.subscribe(async (objects) => {
            ...
            const releasedPromises = await this.genCollections(
                this.learningObjectsReleased,
            );
            const unreleasedPromises = await this.genCollections(
                this.learningObjectsUnreleased,
            );
            ...
            await Promise.allSettled(unreleasedPromises).then(
                (promise: any) => {
                    promise.map((p) => {
                        if (p.status === "fulfilled") {
            ...
            this.collectionsReleased.sort((a, b) =>
                a.name.localeCompare(b.name),
            );
            this.collectionsUnreleased.sort((a, b) =>
                a.name.localeCompare(b.name),
            );
            // Default first released collection tab values
            if (this.collectionsReleased.length) {
    ...
    content(status: number, collection: string) {
        if (status === 1) {
            this.learningObjectsReleased = this.learningObjects.filter(
                (learningObject: any) => {
                    return (
                        learningObject.status === "released" &&
                        learningObject.collection === collection
                    );
                },
            );
        } else if (status === 2) {
            if (collection === "Drafts") {
                this.learningObjectsUnreleased = this.learningObjects.filter(
                    (learningObject: any) => {
                        return (
                            learningObject.status !== "released" &&
                            (learningObject.collection === "" ||
                                learningObject.collection === "Drafts")
                        );
                    },
                );
            } else {
        ...
        for (let i = 0; i < uniqueCollectionNames.length; i++) {
            if (uniqueCollectionNames[i] !== "Drafts") {
                await this.collectionService
                    .getCollection(uniqueCollectionNames[i])
                    .then((c) => {
