...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        ...
        setTimeout(async () => {
            await this.getReleasedLearningObjects({
            ...
            .subscribe(async (params) => {
                ...
                if (cuid && version >= 0) {
                    ...
                    p.then(() => {
                        ...
                        const object = this.releasedLearningObjects.filter(
                            (o) => {
     ...
     * @param query
     */
    async getReleasedLearningObjects(
        filters?: any,
        text?: string,
    ): Promise<void> {
        this.searchService
    ...
    performSearch(text: string) {
        this.getReleasedLearningObjects(
            { status: LearningObject.Status.RELEASED },
            text,
        );
    ...
    async openViewAllChangelogsModal(learningObjectId: string) {
        ...
        this.learningObjects = this.workingLearningObjects.concat(
            this.releasedLearningObjects,
        );
        this.changelogLearningObject = this.learningObjects.find(
            (learningObject) => learningObject.id === learningObjectId,
        );
        try {
            this.changelogs = await this.changelogService.getAllChangelogs(
                this.changelogLearningObject.cuid,
            );
        } catch (error) {
            let errorMessage;
            ...
            if (error.status === 401) {
                ...
                errorMessage =
                    "We encountered an error while attempting to retrieve change logs for this Learning Object. Please try again later.";
            }
    ...
    async deleteObjects(objects: any): Promise<void> {
        ...
        const canDelete = this.objectsToDelete.filter((s) =>
            [
        ...
        if (canDelete.length === 1) {
            return this.learningObjectService
                ...
                .catch((err) => {
                    this.notificationService.error(
                        "Error!, Learning Object could not be deleted! ",
                        err.error.message,
                    );
                });
            ...
            return this.learningObjectService
                .deleteMultiple(objectsToDeleteIDs)
                .then(async () => {
                    this.notificationService.success(
                        "Done!",
    ...
    createRevision(object: LearningObject) {
        this.sidePanelPromiseResolver = this.editorialService
            .createRevision(object.cuid)
            .then(() => {
                this.getReleasedLearningObjects({
                    status: LearningObject.Status.RELEASED,
                });
     ...
     */
    private sortLearningObjectsByDate(
