...
import {
    ...
    sidePanelEnter,
    buttonWipeRight,
} from "./side-panel-content.animations";
...
export class SidePanelContentComponent implements OnChanges, OnDestroy {
    ...
    @Output() cancelRevisionSubmission: EventEmitter<LearningObject> =
        new EventEmitter();
    ...
    createRevisionHandler() {
        ...
        this.promiseResolver.pipe(take(1)).subscribe((promise) => {
            promise
                .then((revisionUri) => {
    ...
    close(shouldRoute: boolean = true) {
        this.el.nativeElement.dispatchEvent(
            new CustomEvent("SidePanelCloseEvent", {
                bubbles: false,
                detail: { shouldRoute: shouldRoute },
            }),
        );
    }
