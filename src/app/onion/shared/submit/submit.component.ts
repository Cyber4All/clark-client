...
export class SubmitComponent implements OnInit {
    ...
    createChangelog(changelog: string) {
        if (changelog) {
            ...
            this.changelogService
                .createChangelog(this.learningObject.cuid, changelog)
                .then(() => {
    ...
    private buildUnfinishedLOErrorMsg(): string {
        ...
        if (this.learningObject.description === "") {
            str +=
                str === ""
                    ? potentialErrorFields["description"]
                    : "," + potentialErrorFields["description"];
        }
        if (this.learningObject.outcomes.length === 0) {
            str +=
                str === ""
                    ? potentialErrorFields["outcome"]
                    : "," + potentialErrorFields["outcome"];
        }
        if (this.learningObject.contributors.length === 0) {
            str +=
                str === ""
                    ? potentialErrorFields["contributor"]
                    : "," + potentialErrorFields["contributor"];
        }
        return str;
    }
        ...
        if (proceed) {
            ...
            if (this.isHierarchySubmission) {
                this.learningObjectService
                    .submitHierarchy(this.learningObject.id, this.collection)
                    .then(() => {
    ...
    getCollectionSelected(collection: string) {
        this.submissionService
            .getFirstSubmission(this.learningObject.id, collection)
            .then((response) => {
    ...
    async setCollections() {
        ...
        ).filter((collection) => collection.abvName !== "nccp");
    }
}
