            ...
            this.activeOutcomeSubscription = this.store.outcomeEvent
                ...
                .subscribe((outcome: LearningOutcome) => {
                    ...
                    if (outcome.verb && outcome.verb !== "") {
                        ...
                        const tempSuggestString =
                            outcome.verb + " " + outcome.text;
                        ...
                        if (outcome.mappings) {
                            this.selectedOutcomeIDs = outcome.mappings.map(
                                (x) => x.guidelineId,
                            );
                        }
                    } else {
            ...
            this.guidelineService
                ...
                .then((res) => {
                    if (this.suggestions.length) {
                        ...
                        const suggestedIds: string[] = this.suggestions.map(
                            (o) => o._id,
                        );
                        this.searchResults = res.results.map((searchItem) => {
                            ...
                            searchItem.suggested = suggestedIds.includes(
                                searchItem._id,
                            );
                            return searchItem;
                        });
