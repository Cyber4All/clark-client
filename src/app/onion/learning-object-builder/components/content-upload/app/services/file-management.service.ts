...
export class FileManagementService {
     ...
     * @param {string} learningObjectCuid [The id of the Learning Object]
    ...
    private validateFileNames(files: FileInput[], cuid: string) {
        ...
        const invalidFilePaths = files
            ...
            .map(
                (file) => file.fullPath || file.webkitRelativePath || file.name,
            );
    ...
    private async getCognitoIdentityId(username: string): Promise<string> {
        ...
        const cognitoIdentityId =
            await this.userService.getUserFileAccessId(username);
    ...
    private async startUploads(
        ...
        const maxConcurrent =
            maxConcurrentUploads || DEFAULT_CONCURRENT_UPLOADS;
    ...
    private processUploadQueue(
        ...
        processNext$.pipe(takeUntil(jobsComplete$)).subscribe(() => {
            const remainingQueueSpace =
                maxConcurrentUploads - enqueueStatus.enqueued;
