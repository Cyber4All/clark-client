...
export class UploadComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        ...
        fromEvent(document.getElementsByTagName("body")[0], "dragleave")
            ...
            .subscribe((event: any) => {
                if (
                    event.target &&
                    event.target.classList.contains("uploader")
                ) {
    ...
    private handleQueueComplete(update: UploadQueueCompleteUpdate) {
        if ((update as UploadQueueCompleteUpdate).data.failed) {
            const unUploadedFiles = this.uploadQueue.filter(
                (file) => !file.success,
            );
            const fileNames = unUploadedFiles
                .map((file) => file.name)
                .join(", ");
    ...
    private handleCredentialsError() {
        if (!this.credentialRefreshAttempted) {
            ...
            this.auth
                .refreshToken()
                .then(() => {
                    const failedFiles = this.uploadQueue.filter(
                        (file) => !file.success,
                    );
                    this.resetUploadStatuses();
    ...
    async handleFileDownload(file: LearningObject.Material.File) {
        const learningObject = await this.learningObject$
            .pipe(take(1))
            .toPromise();
        await this.fileService.handleFileDownload(file, learningObject);
    }
    ...
    async *handleDeletion(files: string[]) {
        ...
        if (confirmed) {
            ...
            try {
                ...
                const object = await this.learningObject$
                    .pipe(take(1))
                    .toPromise();

                // Delete each file
                files.forEach(async (fileId) => {
                    await this.fileService.deleteLearningObjectFileMetadata(
                        object.id,
                        fileId,
                    );
                });
