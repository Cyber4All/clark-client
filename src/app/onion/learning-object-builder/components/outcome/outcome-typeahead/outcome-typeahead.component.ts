...
@Component({
    ...
    standalone: true,
    imports: [NgIf, NgClass, ActivateDirective, NgFor, TitleCasePipe],
})
export class OutcomeTypeaheadComponent implements OnInit, OnChanges, OnDestroy {
                ...
                if (val !== this.verb) {
                    ...
                    if (!this.verb) {
                        ...
                        if (index >= 0) {
                            ...
                            this.verb =
                                this.verb.charAt(0).toUpperCase() +
                                this.verb.substring(1); // capitalize the first letter of the verb
    ...
    isGoodVerb(verb: string, noEmit?: boolean): boolean {
        if (!this.bloom) {
            ...
            const match = entries.find(([_, taxon]) =>
                taxon.verbs.includes(verb.toLowerCase()),
            );
        ...
        for (let i = 0, l = levelsArray.length; i < l; i++) {
            if (levelsArray[i] === this.bloom) {
                if (
                    taxonomy.taxons[levelsArray[i]].verbs.includes(
                        verb.toLowerCase(),
                    )
                ) {
