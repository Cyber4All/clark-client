...
import { LearningObject, LearningOutcome, User, Guideline } from "@entity";
...
export class BuilderStore {
    ...
    private outcomeCache$: BehaviorSubject<any> = new BehaviorSubject(
        undefined,
    );
    ...
    public learningObjectEvent: BehaviorSubject<LearningObject> =
    ...
    public serviceInteraction$: BehaviorSubject<boolean> =
    ...
    private set learningObject(object: LearningObject) {
        ...
        if (![LearningObject.Status.UNRELEASED].includes(object.status)) {
    ...
    fetch(
        ...
        const retrieve =
            this._isRevision && revisionId !== undefined && username
                ? async () => {
                      // eslint-disable-next-line eqeqeq
                      if (revisionId == 0) {
                          revisionId =
                              await this.editorialService.createRevision(cuid);
                      }

                      return this.refactoredLearningObjectService.getLearningObject(
                          cuid,
                          revisionId,
                      );
                  }
                : async () => {
                      const value = this.uriRetriever.getLearningObject(
        ...
        return retrieve()
            .then((object) => {
                ...
                this.validator.submissionMode =
                    ...
                    ![LearningObject.Status.UNRELEASED].includes(
                        this.learningObject.status,
                    );
                this.outcomes = this.parseOutcomes(
                    this.learningObject.outcomes,
                );
        ...
        this.newLearningObjectService
            ...
            .catch((e) => {
                this.handleServiceError(
                    e,
                    BUILDER_ERRORS.FETCH_OBJECT_MATERIALS,
                );
            });
    }
    ...
    async setChildren(children: string[], remove: boolean = false) {
        ...
        if (remove) {
            children = this.learningObject.children
                .filter((child) => !children.includes(child.id))
                .map((child) => child.id);
        }
        await this.refactoredLearningObjectService.setChildren(
    ...
    private parseOutcomes(outcomes: LearningOutcome[]) {
        return new Map(
            outcomes.map((outcome): [string, LearningOutcome] => [
                outcome.id,
                outcome,
            ]),
        );
    ...
    async execute(action: BUILDER_ACTIONS, data?: any): Promise<any> {
        switch (action) {
            ...
            case BUILDER_ACTIONS.UPDATE_FILE_DESCRIPTION:
                return await this.updateFileDescription(
                    data.id,
                    data.description,
                );
     ...
     */
    async toggleBundle(event: { state: boolean; item: any }) {
        if (event.item instanceof DirectoryNode) {
            // event.item is a Folder
    ...
    private deleteOutcome(id: string) {
        ...
        if (
            !checkIfUUID(outcome.serviceId || outcome.id) &&
            (outcome.serviceId || outcome.id)
        ) {
            this.serviceInteraction$.next(true);
            this.outcomeService
                ...
                .catch((e) =>
                    this.handleServiceError(e, BUILDER_ERRORS.DELETE_OUTCOME),
                );
        }
    }
            ...
            this.saveOutcome(
                {
                    ...
                    text: outcome.text,
                    serviceId: (
                        outcome as Partial<LearningOutcome> & {
                            serviceId?: string;
                        }
                    ).serviceId,
                },
    ...
    private mapStandardOutcomeMapping(id: string, guideline: Guideline) {
        const outcome = this.outcomes.get(id);
    ...
    private unmapStandardOutcomeMapping(id: string, standardOutcome: any) {
        const outcome = this.outcomes.get(id);
    ...
    private mutateObject(data: any) {
        ...
        const learningObject: Partial<LearningObject> =
    ...
    private addContributor(user: User) {
        ...
        this.saveObject(
            {
                contributors: this.learningObject.contributors.map(
                    (x) => x.userId,
                ),
            },
            true,
    ...
    private removeContributor(user: User) {
        ...
        if (index >= 0) {
            this.learningObject.removeContributor(
                this.learningObject.contributors.indexOf(user),
            );
            this.validator.validateLearningObject(this.learningObject);

            const message =
                this.validator.errors.submitErrors.get("contributors");
            if (message) {
    ...
    private updateFileDescription(fileId: any, description: any): void {
        ...
        this.fileService
            .updateFileDescription(this.learningObject.id, fileId, description)
            .then((e) => {
                this.learningObjectEvent.next(this.learningObject);
            })
            .catch((e) =>
                this.handleServiceError(
                    e,
                    BUILDER_ERRORS.UPDATE_FILE_DESCRIPTION,
                ),
            );
    }
    ...
    private updateFolderDescription(params: {
        ...
        const index =
            this.learningObject.materials.folderDescriptions.findIndex(
    ...
    public cancelSubmission(): void {
        this.submissionService
            .unsubmitLearningObject(this.learningObject.id)
            .then(() => {
    ...
    public async removeEmptyOutcomes() {
        ...
        const value = await this.uriRetriever
            .getLearningObject(
                { cuidInfo: { cuid: this.learningObject.cuid } },
                ["outcomes"],
            )
            .toPromise();
