...
export class LearningObjectBuilderComponent
    ...
    ngAfterViewInit(): void {
        this.routeState =
            this.route.snapshot.firstChild?.data?.["state"] ?? "default";
    ...
    ngOnInit() {
        ...
        this.route.paramMap
            ...
            .subscribe((routeParams) => {
                ...
                const revision =
                    this.route.snapshot.queryParamMap.get("revisionId");
                ...
                if (revision !== undefined && cuid) {
                    ...
                    this.store.fetch(cuid, version).then((learningObject) => {
                        if (
                            ...
                            if (
                                this.isInReviewStage(learningObject) &&
                                !this.authService.hasEditorAccess
                            ) {
                                this.router.navigate(["onion/dashboard"]);
                            } else {
                                this.setBuilderMode(learningObject);
    ...
    private handleBuilderError(error: BUILDER_ERRORS) {
        ...
        switch (error) {
            ...
            case BUILDER_ERRORS.ADD_FILE_META:
                this.noteService.error(toasterTitle, "Unable to add file(s)");
                break;
            default:
                break;
        }
