...
export class LearningObjectBuilderComponent implements OnInit {
    ...
    loadLearningObject(): void {
        this.service.getLearningObject(this.learningObjectName)
            .then((learningObject) => {
    ...
    save(willUpload: boolean): void {
        ...
        if (!this.isNew) {
            this.service.save(this.learningObject)
                .then(success => {
                    this.notificationService.notify('Done!', 'Learning Object saved!', 'alerting', 'fal fa-save');
                    willUpload ? this.router.navigateByUrl(`/onion/content/upload/${this.learningObjectName}`) : this.router.navigate(['/onion']);
                })
                .catch((err) => {
                    const error = (typeof err['_body'] == 'string') ? err['_body'] : 'Error saving Learning Object';
                    this.notificationService.notify('Error!', error, 'bad', 'far fa-times');
                });
        } else {
            this.service.create(this.learningObject)
                .then(() => {
                    this.notificationService.notify('Done!', 'New Learning Object created!', 'good', 'far fa-check');
                    willUpload ? this.router.navigateByUrl(`/onion/content/upload/${this.learningObject.name}`) : this.router.navigate(['/onion']);
                })
                .catch((err) => {
                    const error = (typeof err['_body'] == 'string') ? err['_body'] : 'Error creating Learning Object';
                    this.notificationService.notify('Error!', error, 'bad', 'far fa-times');
                });
        }
    }
    ...
    newOutcome() {
        ...
        if (newOutcome.verb === 'Define') { newOutcome.verb = 'Choose'; }
        return newOutcome;
    };
    ...
    showFormErrors() {
        this.submitted++;
        this.notificationService.notify('Error!', 'Please complete all required fields!', 'bad', 'fal fa-times');
    }
    ...
    validate(): boolean {
        ...
        // check outcomes
        let o: NodeListOf<Element> = document.querySelectorAll('learning-outcome-component > .container');
