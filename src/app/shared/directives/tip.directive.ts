...
export class TipDirective implements OnChanges, OnDestroy {
    ...
    hideTooltip(event?) {
        ...
        const nextTarget: HTMLElement = event
    ...
    positionTooltip(targetElement: HTMLElement) {
        ...
        const positionTop = () => {
            this.element.style.top =
                Math.max(10, top - this.element.offsetHeight - 8) + "px";

            // center horizontally
            const centeredLeft = Math.max(
                10,
                left - (this.element.offsetWidth - width) / 2,
            );
      ...
      `;
        ...
        const positionRight = () => {
            if (
                window.innerWidth - (right + 8 + this.element.offsetWidth) >
                10
            ) {
            ...
            // center vertically
            this.element.style.top =
                top - (this.element.offsetHeight - height) / 2 + "px";
      ...
      `;
        ...
        const positionLeft = () => {
            this.element.style.left =
                Math.max(10, left - this.element.offsetWidth - 8) + "px";
            // center vertically
            this.element.style.top =
                top - (this.element.offsetHeight - height) / 2 + "px";
      ...
      `;
        ...
        const positionBottom = () => {
            if (
                window.innerHeight - (bottom + 8 + this.element.offsetHeight) >
                10
            ) {
        ...
        switch (this.tipPosition) {
            ...
            default:
                ...
                console.error(
                    "Error! `" +
                        this.tipPosition +
                        "` isn't a valid position! Using `top` instead!",
                );
        }
    }
