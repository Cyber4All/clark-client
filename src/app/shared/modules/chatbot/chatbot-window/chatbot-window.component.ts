...
@Component({
    ...
    standalone: true,
    imports: [MatTooltip, NgFor, MarkdownComponent, NgIf, FormsModule],
})
export class ChatbotWindowComponent implements AfterViewInit {
    ...
    async sendMessage(): Promise<void> {
        ...
        try {
            // Send to backend
            const response = (await this.chatbotService.sendPrompt(
                userMessage,
            )) as {
                message: string;
                sessionId: string;
    ...
    maybePromptForFeedback(force: boolean = false): void {
        ...
        const alreadyPrompted =
            localStorage.getItem("clark_ai_feedback_prompted") === "true";
        const alreadySubmitted =
            localStorage.getItem("clark_ai_feedback_submitted") === "true";
