...
export class LoginComponent implements OnInit {
    ...
    constructor(
        ...
        this.route.parent.data.subscribe(() => {
            ...
            if (this.route.snapshot.queryParams.err) {
                this.bannerMsg = decodeURIComponent(
                    this.route.snapshot.queryParams.err,
                );
    ...
    ngOnInit(): void {
        this.authValidation
            .getErrorState()
            .subscribe((err) => (this.loginFailure = err));
    }
    ...
    public async submit(form: NgForm) {
        ...
        if (
            usernameFormCtl.hasError("required") ||
            pwordFormCtl.hasError("required")
        ) {
            this.bannerMsg = "Please fill out all required fields";
            this.authValidation.showError();
        } else if (
            this.attempts < 5 ||
            this.hourPassed(Date.parse(new Date().toDateString()))
        ) {
            this.attempts++;
