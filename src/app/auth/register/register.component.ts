...
import { COUNTRY_OPTIONS, US_STATE_OPTIONS } from "./register-location-options";
...
export class RegisterComponent implements OnInit, OnDestroy {
    ...
    errorMsg =
        "There was an issue on our end with your registration, we are sorry for the inconvience.\n Please try again later!";
    fieldErrorMsg = "";
    redirectUrl;
    ...
    get canEnterCustomOrganizationFlow(): boolean {
        return (
            this.infoFormGroup.get("firstname")!.valid &&
        ...
        this.accountFormGroup = new UntypedFormGroup(
            {
                ...
                confirmPassword:
                    this.authValidation.getInputFormControl("required"),
    ...
    ngOnInit(): void {
        this.authValidation
            .getErrorState()
            .subscribe((err) => (this.registrationFailure = err));
        ...
        this.organizationInput$
            ...
            .subscribe((value: string) => {
                this.orgService
                    .searchOrganizations({ text: value.trim() })
                    .pipe(take(1))
                    .subscribe({
                        next: (results) => {
        ...
        this.organizationInput$.subscribe((value: string) => {
            if (value && value !== "") {
                ...
                this.regInfo.organization =
                    this.suggestedOrganization?.name ?? "";
                this.showDropdown = false;
                this.organizationSearchLoading = false;
            }
        });
    ...
    private validateEmail() {
        this.infoFormGroup
            ...
            .subscribe(async (value) => {
                ...
                if (
                    !this.emailInUse &&
                    !this.infoFormGroup.get("email").errors
                ) {
    ...
    private toggleOrganizationNextButton() {
        this.organizationFormGroup.statusChanges
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe((value) => {
                this.buttonGuards.isOrganizationPageInvalid =
                    value === "INVALID";
            });
    }
    ...
    toggleOrganizationLevel(level: OrganizationLevel): void {
        const currentLevels: OrganizationLevel[] =
            this.organizationFormGroup.get("levels")!.value || [];
    ...
    private getErrorMessage(error: unknown): string | null {
        ...
        if (typeof error === "object") {
            ...
            if (
                Array.isArray(maybeError.message) &&
                maybeError.message.length > 0
            ) {
                const firstMessage =
                    maybeError.message[0]?.message || maybeError.message[0];
                if (firstMessage) {
                    return firstMessage;
                }
            }
            ...
            .subscribe({
                next: (response: SuggestDomainResponse) => {
                    ...
                    if (this.suggestedOrganization) {
                        ...
                        this.infoFormGroup
                            .get("organization")!
                            .setValue(this.suggestedOrganization.name);
                    }
                },
                error: () => {
                    if (
        ...
        try {
            const response = (await this.orgService
                .createOrganization({
                    ...
                    sector: this.organizationFormGroup.get("sector")!
                        .value as OrganizationSector,
                    levels: this.organizationFormGroup.get("levels")!
